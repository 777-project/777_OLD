    ...
    isLoading(false)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can delegate your 777s, letting a hot node (24/7 online node)\nstake on your behalf, while you keep the keys securely offline."));
    ...
    ui->labelSubtitle2->setText(tr("Accept 777 delegation / Delegate 777"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select 777 outputs to delegate.");
...
void ColdStakingWidget::onCoinControlClicked()
{
    if (isInDelegation) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any 777 to select."));
        }
    }
}
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 777" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}
