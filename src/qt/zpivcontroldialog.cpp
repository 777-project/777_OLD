    ...
    model(0)
{
    ...
    // Title
    ui->labelTitle->setText(tr("Select z777 Denominations to Spend"));
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < nRequiredConfs)
                ...
                strReason = "Your wallet is locked. Impossible to spend z777.";
            else if (!mint.isSeedCorrect)
                strReason = "The z777 seed used to mint this z777 is not the same as currently hold in the wallet";
            else
