        ...
        if (zwalletMain) {
            ...
            if (CWalletDB(strWalletFile).ReadCurrentSeedHash(hashSeed)) {
                ...
                if (!GetDeterministicSeed(hashSeed, nSeed)) {
                    return error("Failed to read z777 seed from DB. Wallet is probably corrupt.");
                }
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 777 collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 777 collateral
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-jackpot-address
                    CScript scriptChange;
...
bool CWallet::CreateCoinStakeKernel(CScript &kernelScript, const CScript &stakeScript, unsigned int nBits, const CBlock &blockFrom, const CTransaction &txPrev, const COutPoint &prevout, unsigned int &nTimeTx, bool fPrintProofOfStake) const
{
    ...
    for(unsigned int i = 0; i < nHashDrift; ++i)
    {
        ...
        if (CheckStakeKernelHash(nBits, blockFrom, txPrev, prevout, nTryTime, nHashDrift, true, hashProofOfStake)) {
            ...
            if (nTryTime > GetAdjustedTime() + Params().GetConsensus().nFutureTimeDriftPoS)
            {
